...
with st.sidebar:
    ...
    job_titles = ["Semua"] + sorted(job["job_title"].dropna().unique().tolist())
    sel_title  = st.selectbox("Job Title", job_titles)

    exp_levels = ["Semua"] + sorted(job["experience_level"].dropna().unique().tolist())
    sel_exp    = st.selectbox("Experience Level", exp_levels)

    st.markdown("---")
    st.markdown("**Filter Kandidat**")
    match_cats  = ["Semua"] + sorted(cand["match_category"].dropna().unique().tolist())
...
avg_score   = cand_f["matched_score"].mean() if not cand_f.empty else 0
high_pct    = ((cand_f["match_category"] == "High Match").mean() * 100) if not cand_f.empty else 0
...
for col, label, val, sub in [
    ...
    (k4, "Avg Matched Score",     f"{avg_score:.2f}",    "kandidat terfilter"),
...
with tabs[0]:
    ...
    with col_a:
        ...
        if not job_f.empty and "kkni_level_mapped" in job_f.columns:
...
with tabs[1]:
    ...
    all_titles = sorted(job["job_title"].dropna().unique().tolist())
    skill_rows = []
    ...
    # ── DETAIL INTERAKTIF DINAMIS BERDASARKAN SELECTION ──
    st.markdown("---")
    col_det1, col_det2 = st.columns([1, 2])
    
    with col_det1:
        st.markdown("##### 🔍 Pilih Job Title Detail")
        sel_detail = st.selectbox("Pilih peran untuk melihat profil radar & bar detail:", all_titles, key="bq2_select")
        
        sub_detail = job[job["job_title"] == sel_detail]
        n_jobs = len(sub_detail)
        st.metric("Jumlah Lowongan Teranalisis", f"{n_jobs} loker")
    ...
    with col_det2:
        ...
        fig_detail = go.Figure(go.Bar(
            ...
            text=[f"{v} ({v/max(n_jobs,1)*100:.0f}%)" for v in freq.values],
        ...
        fig_detail.update_layout(
            ...
            xaxis_range=[0, max(freq.values) * 1.3 if max(freq.values) > 0 else 10],
            margin=dict(t=30, b=10), height=280,
            title=f"Spesifikasi Kebutuhan Kompetensi · {sel_detail}",
        )
        st.plotly_chart(fig_detail, use_container_width=True)

    # ── Radar chart keseluruhan ──
    st.markdown("##### Radar: Perbandingan Profil Kompetensi Antar Peran")
    fig_radar = go.Figure()
    colors_radar = [C_PRIMARY, C_GREEN, C_ORANGE, C_RED, C_DARK, "#a78bfa"]
    for i, title in enumerate(all_titles):
        d = skill_df[skill_df["job_title"] == title]
        if not d.empty:
...
with tabs[2]:
    ...
    if not cand_f.empty:
        ...
        with col_a:
            ...
            m_score = cand_f["matched_score"].mean()
            ...
            fig_hist.add_vline(x=m_score, line_dash="dash", line_color=C_DARK,
                               annotation_text=f"Mean: {m_score:.2f}", annotation_position="top right")
        ...
        st.info("Tidak ada data kandidat yang memenuhi kriteria filter saat ini.")
...
with tabs[3]:
    ...
    if not cand_f.empty and "overlap_with_job_skills" in cand_f.columns:
        col_a, col_b = st.columns(2)
        ...
        with col_a:
            ...
            overlap_dist["overlap_label"] = overlap_dist["overlap"].astype(str) + " skill"
            ...
            fig_ov_bar.update_layout(
                ...
                plot_bgcolor="#0f1117", paper_bgcolor="#0f1117",
                font_color="#e6e6e6",
                xaxis_title="Jumlah Skill Cocok", yaxis_title="Jumlah Kandidat",
                margin=dict(t=20, b=10), height=320,
                hovermode="x unified"
            )
        ...
        with col_b:
            ...
            overlap_score = cand_f.groupby("overlap_with_job_skills")["matched_score"].agg(["mean", "count"]).reset_index()
            ...
            fig_line.add_trace(go.Scatter(
                ...
                text=overlap_score["avg_score"].round(2).astype(str),
        ...
        ov_cat = cand_f.groupby(["overlap_with_job_skills", "match_category"]).size().reset_index(name="count")
        ...
        for cat in ["High Match", "Mid Match", "Low Match"]:
            if cat in ov_cat_pct.columns:
                fig_cat_stack.add_trace(go.Bar(
                    x=ov_cat_pct.index.astype(str) + " Skill",
                    y=ov_cat_pct[cat],
                    name=cat,
        ...
        st.info("Data overlap skill tidak memadai.")
...
with tabs[4]:
    ...
    if "kkni_level_inferred" in cand_f.columns:
        pontik_dist = pontik.groupby(["kkni_level", "kkni_label"]).size().reset_index(name="n_roles")
        cand_dist = cand_f["kkni_level_inferred"].dropna().astype(int).value_counts().reset_index()
        ...
        gap_df["gap"] = gap_df["n_roles"] - gap_df["n_candidates"]
        ...
        with col_a:
            ...
            if not gap_plot.empty:
                fig_bubble = px.scatter(
                    gap_plot,
                    x="kkni_label", y="gap_ratio",
                    size="n_roles", color="gap_ratio",
                    color_continuous_scale="RdYlGn_r",
                    text="gap_ratio",
                    size_max=50,
                )
                fig_bubble.update_traces(textposition="top center")
                fig_bubble.add_hline(y=1, line_dash="dot", line_color="white", annotation_text="Seimbang (1:1)")
                fig_bubble.update_layout(
                    plot_bgcolor="#0f1117", paper_bgcolor="#0f1117",
                    font_color="#e6e6e6", xaxis=dict(tickangle=-20),
                    coloraxis_showscale=False, margin=dict(t=20, b=10), height=340,
                )
                st.plotly_chart(fig_bubble, use_container_width=True)
            else:
                st.info("Rasio gap tidak dapat dikalkulasi (kandidat bernilai 0).")
        ...
        with col_b:
            ...
            st.dataframe(gap_show.reset_index(drop=True), use_container_width=True, hide_index=True, height=340)
        ...
        fig_wf = go.Figure(go.Bar(
            x=gap_df["kkni_label"], y=gap_df["surplus"],
        ...
        st.info("Data kolom KKNI Inferred tidak ditemukan.")
